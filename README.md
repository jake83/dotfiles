...
- **Junie boots with Brave Mode on**, via `home/.junie/config.json` (`{"brave": true}`). Only that one file is symlinked, never the whole `~/.junie` directory: Junie rewrites `~/.junie/settings.json` on every launch and keeps its logs, sessions and sockets alongside it. Startup precedence is `--brave` flag, then `braveMode` in `settings.json`, then `brave` in `config.json`, then the built-in default (`AUTO`). So the in-app toggle (`/brave`, `Ctrl+B`) is sticky and outranks this file on a machine where it has ever been used; `config.json` is what gets a fresh machine right. Unknown keys are ignored silently, so a typo like `braveMode` here would do nothing.
